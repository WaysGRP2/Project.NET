    ...
    class WF_Get_Questions_Jeu : IWorkflow
    {
        public Message Exec(Message msg)
        {
            ...
            foreach (System.Data.DataRow row in results.Tables[0].Rows)
            {
                ...
                {
                    int idRep = Convert.ToInt32(row[Mpg_Reponses_Jeu.CH_ID].ToString());
                    int idQues = Convert.ToInt32(row[Mpg_Reponses_Jeu.CH_ID_QUESTION].ToString());
                    string intituleRep = row[Mpg_Reponses_Jeu.CH_INTITULE].ToString();
                    int points = Convert.ToInt32(row[Mpg_Reponses_Jeu.CH_POINTS].ToString());
